# Mini-Quiz #2 — Topics 6–9 (Difference of Squares + All Graphing)
**For Day 11 (Wednesday, May 27, 2026)**
**Suggested time:** 25 min. Cheat sheet allowed.
**Why this quiz focuses on graphing:** Mr. Mistry has flagged repeatedly that graphing is where Esme loses the most test points. This is the diagnostic for the area that matters most.

---

## Part A — Difference of Squares (Topic 6) — 4 pts

1. (2) Factor: `x² − 64`
2. (2) Factor: `9x² − 49`

## Part B — Graphing Basics (Topic 7) — 4 pts

3. (4) For `y = 3x²`:
   - Vertex: __________
   - Opens up or down? __________
   - Wider or narrower than y = x²? __________
   - y when x = 2? __________

## Part C — Intercept Form (Topic 8) — 6 pts

4. (3) For `y = (x − 4)(x + 2)`:
   - x-intercepts: __________
   - Axis of symmetry: __________
   - Vertex: __________

5. (3) For `y = −(x − 1)(x − 5)`:
   - x-intercepts: __________
   - Vertex: __________
   - Opens up or down: __________

## Part D — Vertex Form (Topic 9) — 6 pts

6. (3) For `y = (x − 5)² + 2`:
   - Vertex: __________
   - AOS: __________
   - y when x = 3? __________

7. (3) For `y = −2(x + 3)² + 4`:
   - Vertex: __________
   - AOS: __________
   - Opens up or down: __________

**Total: 20 pts**

---

## ✅ Answer Key

1. `(x + 8)(x − 8)`
2. `(3x + 7)(3x − 7)`
3. Vertex **(0, 0)**, opens **up**, **narrower**, y(2) = **12**
4. x-intercepts: **(4, 0)** and **(−2, 0)** (sign flips!), AOS **x = 1**, vertex: plug x=1 → (1−4)(1+2) = (−3)(3) = −9 → **(1, −9)**
5. x-intercepts: **(1, 0)** and **(5, 0)**, vertex: AOS = 3, y = −(3−1)(3−5) = −(2)(−2) = 4 → **(3, 4)**, opens **down**
6. Vertex **(5, 2)**, AOS **x = 5**, y(3) = (3−5)² + 2 = 4 + 2 = **6**
7. Vertex **(−3, 4)** (sign flip!), AOS **x = −3**, opens **down** (a is negative)

---

## 📊 What Your Score Means

- **17–20:** 🎉 Graphing is solid. The rest of the course should flow.
- **13–16:** Almost there. Note which form you wobbled on. Re-do those examples in the topic file.
- **9–12:** Need targeted graphing work. Spend extra time on Topic 9 (Vertex Form) — most leverage.
- **Below 9:** Schedule a 45-min co-work block with Nate this weekend on graphing. We have time.

---

## 🚨 If You Got Any Sign-Flip Errors (very common)

Re-read these two cards:
- **Vertex form:** `(x − 5)²` → h = +5. `(x + 5)²` → h = −5. **Sign inside flips.**
- **Intercept form:** `(x − 5)` → root at +5. `(x + 5)` → root at −5. **Sign flips.**

This is the #1 test trap. If you ace this on the real test, you'll be ahead of most of the class.

---

*Now you know what to drill in Week 3. The information is the gift. — Day 11 of 19.*
